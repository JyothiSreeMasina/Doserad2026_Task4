...
# do_test_run.sh
#
# Rebuilds the Task 2 (photon/MR) submission image, boots it as an HTTP
# server implementing Grand Challenge's "invoke" API, stages
# docker_task2_photon_mr/test-data/input (patient 1ABB006's real MR --
# same patient as Task 1's CT test data, so the reused beam metadata's
# iso_center/geometry is spatially consistent -- plus 4 real control
# points, and 1x1x1 placeholders in the other 9 image slots, matching the
# real batched contract), calls /invoke once, and collects output into
# docker_task2_photon_mr/test-data/output.
#
...
#
# NOTE: the image-slot directory naming (radiation-dose-calculation-source-
# mr-image-N) is an inferred-by-analogy guess, not platform-confirmed -- see
# process.py's module docstring. This test proves the container's own logic
# works; it does NOT prove the socket names match the real Grand Challenge
# "Photon Dose on MR" algorithm interface.
...
CONTAINER_NAME="doserad2026_task2_photon_mr_test"
CONTAINER_PORT=4743  # app.py hardcodes this internally, same as Task 1
HOST_PORT=4745  # a leftover Task 1 test container was still holding host:4743 -- use a different host port to avoid it, no need to touch that container
